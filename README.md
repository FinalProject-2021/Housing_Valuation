...
We also employed a number of approaches to time series modelling, including a random forest regressor, ARIMA, and VAR (vector autoregressive modelling). VAR has the advantage of allowing the use of multiple features.
...
We ran ARIMA tests to predict house prices, total house sales, and # of FHA loans in the state and by zip code. We ran autocorrelation tests to gauge the statistical significance of different time lags. For house prices, we ended up settling on 5 months of lag, which had significance above a reasonable threshold.  The results are detailed in Tableau, but they include predictions like the following: For April 2021, our ARIMA model predicts the mean home sales price across Florida to be $287,972.77.

### Potential Next Steps

On Tableau, users can do some exploratory data analysis. A next step for this project might be to build in tools that allow users to select features to include in runs of machine learning models, and to facilitate running the models in real-time with those user inputs.
